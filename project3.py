...
def find_closest(location, centroids):
    ...
    distances = {}
    for x in centroids:
        compare = distance(location, x)
        distances[tuple(x)] = compare
    min_key = min(distances, key = lambda w: distances[w])
    return list(min_key)
...
def group_by_centroid(restaurants, centroids):
    ...
    for c in centroids:
        for i in restaurants:
            loc = restaurant_location(i)
            closest_centroid = find_closest(loc, centroids)
            if closest_centroid == c:
                lst.append([closest_centroid, i])
...
def find_centroid(cluster):
    ...
    for i in cluster:
        ...
        latitudes.append(locations[0])
        longitudes.append(locations[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            dict[restaurant_name(r)] = user_score(user, restaurant_name(r))
        else:
